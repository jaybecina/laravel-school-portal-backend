    ...
    <div class="row">
      <div class="col-lg-12 margin-tb">
          ...
          <div class="pull-right">
              <a class="btn btn-success" href="{{ route('admin.news.create') }}"> Create News</a>
          </div>
      </div>
    </div>
  ...
  <div class="table-responsive">
    <table class="table table-striped align-items-center">
      <tr class="py-4">
        ...
        <th class="align-middle text-center text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Author</th>
      ...
      @foreach ($data as $key => $news)
        ...
        <tr>
          <td class="align-middle text-center text-sm">
            <img src={{ url($news->path) }} alt="Image {{ $news->imagename }}" style="width: 50px; height; 50px; object-fit: contain;" />
          </td>
          <td class="align-middle text-center text-sm">{{ $news->name }}</td>
          <td class="align-middle text-center text-sm">{{ truncateText($news->body) }}</td>
          <td class="align-middle text-center text-sm">{{ $news->author }}</td>
          <td class="align-middle text-center text-sm">{{ $news->is_active == 1 ? "Active" : "Inactive" }}</td>
          <td class="d-flex justify-content-center align-items-center align-middle text-center text-sm pb-0">
            ...
            <a class="btn btn-primary mr-2" href="{{ route('admin.news.edit', $news->id) }}">Edit</a>
              {!! Form::open(['method' => 'DELETE','route' => ['admin.news.destroy', $news->id],'style'=>'display:inline']) !!}
                @csrf
