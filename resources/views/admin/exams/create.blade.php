...
<div class="card p-4">
    ...
    {!! Form::open(array('route' => 'admin.exams.store','method'=>'POST', 'enctype'=>'multipart/form-data')) !!}
    @csrf
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            ...
            <div class="input-group input-group-outline mb-3">
                {{-- <label class="form-label">{{ __('Word Document') }}</label> --}}
                <input type="file" class="form-control @error('document') is-invalid @enderror" name="document" value="{{ old('document') }}" placeholder="{{ __('Upload Document') }}" required autocomplete="off" autofocus>
                @error('document')
                <span class="invalid-feedback" role="alert" align=right>
                    <strong>{{ $message }}</strong>
                </span>
