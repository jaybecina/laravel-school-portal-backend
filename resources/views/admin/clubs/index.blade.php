      ...
      @foreach ($data as $key => $club)
        ...
        <tr>
          <td class="align-middle text-center text-sm">
            <img src={{ url($club->path) }} alt="Image {{ $club->imagename }}" style="width: 50px; height; 50px; object-fit: contain;" />
          </td>
          <td class="align-middle text-center text-sm">{{ $club->name }}</td>
          <td class="align-middle text-center text-sm">{{ truncateText($club->details) }}</td>
          <td class="align-middle text-center text-sm">{{ $club->is_active == 1 ? "Active" : "Inactive" }}</td>
          <td class="d-flex justify-content-center align-items-center align-middle text-center text-sm pb-0">
            <a class="btn btn-info" href="{{ route('admin.clubs.show',$club->id) }}">Show</a>
            <a class="btn btn-primary mr-2" href="{{ route('admin.clubs.edit', $club->id) }}">Edit</a>
              {!! Form::open(['method' => 'DELETE','route' => ['admin.clubs.destroy', $club->id],'style'=>'display:inline']) !!}
                @csrf
