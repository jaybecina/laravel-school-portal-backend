  ...
  <div class="card-header px-0 py-2 position-relative z-index-2">
    <div class="row">
      <div class="col-lg-12 margin-tb">
          <div class="pull-left">
              <h2>Virtual Tour Management</h2>
          </div>
          <div class="pull-right">
              <a class="btn btn-success" href="{{ route('admin.virtual-tour.create') }}"> Create Virtual Tour</a>
          </div>
      </div>
    </div>
  ...
  <div class="table-responsive">
    <table class="table table-striped align-items-center">
      <tr class="py-4">
        ...
        <th class="align-middle text-center text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Video ID</th>
      ...
      @foreach ($data as $key => $virtualTour)
        @php
        ...
        <tr>
          <td class="align-middle text-center text-sm">{{ $virtualTour->title }}</td>
          <td class="align-middle text-center text-sm">{{ truncateText($virtualTour->desc) }}</td>
          <td class="align-middle text-center text-sm">{{ $virtualTour->videoId }}</td>
          <td class="align-middle text-center text-sm">{{ $virtualTour->is_active == 1 ? "Active" : "Inactive" }}</td>
          <td class="d-flex justify-content-center align-items-center align-middle text-center text-sm pb-0">
            <a class="btn btn-info" href="{{ route('admin.virtual-tour.show',$virtualTour->id) }}">Show</a>
            <a class="btn btn-primary mr-2" href="{{ route('admin.virtual-tour.edit', $virtualTour->id) }}">Edit</a>
              {!! Form::open(['method' => 'DELETE','route' => ['admin.virtual-tour.destroy', $virtualTour->id],'style'=>'display:inline']) !!}
                @csrf
