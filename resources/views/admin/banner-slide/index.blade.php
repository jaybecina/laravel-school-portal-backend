  ...
  <div class="card-header px-0 py-2 position-relative z-index-2">
    <div class="row">
      <div class="col-lg-12 margin-tb">
          <div class="pull-left">
              <h2>Banner Slide Management</h2>
          </div>
          <div class="pull-right">
              <a class="btn btn-success" href="{{ route('admin.banner-slide.create') }}"> Create Banner Slide</a>
          </div>
      </div>
    </div>
  ...
  <div class="table-responsive">
    <table class="table table-striped align-items-center">
      <tr class="py-4">
        ...
        <th class="align-middle text-center text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Link</th>
      ...
      @foreach ($data as $key => $bannerSlide)
        @php
        ...
        <tr>
          <td class="align-middle text-center text-sm">
            <img src={{ url($bannerSlide->path) }} alt="Image {{ $bannerSlide->imagename }}" style="width: 50px; height; 50px; object-fit: contain;" />
          </td>
          <td class="align-middle text-center text-sm">{{ $bannerSlide->name }}</td>
          <td class="align-middle text-center text-sm">{{ truncateText($bannerSlide->body) }}</td>
          <td class="align-middle text-center text-sm">{{ $bannerSlide->link }}</td>
          <td class="align-middle text-center text-sm">{{ $bannerSlide->is_active == 1 ? "Active" : "Inactive" }}</td>
          <td class="d-flex justify-content-center align-items-center align-middle text-center text-sm pb-0">
            <a class="btn btn-info" href="{{ route('admin.banner-slide.show',$bannerSlide->id) }}">Show</a>
            <a class="btn btn-primary mr-2" href="{{ route('admin.banner-slide.edit', $bannerSlide->id) }}">Edit</a>
              {!! Form::open(['method' => 'DELETE','route' => ['admin.banner-slide.destroy', $bannerSlide->id],'style'=>'display:inline']) !!}
                @csrf
