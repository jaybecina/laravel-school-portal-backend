...
<div class="card p-4">
    ...
    <div class="row">
      <div class="col-lg-12 margin-tb">
          ...
          <div class="pull-right">
              <a class="btn btn-success" href="{{ route('admin.library-resources.create') }}"> Upload New Library Resource</a>
          </div>
      </div>
    </div>
  ...
  <div class="table-responsive">
    <table class="table table-striped align-items-center">
      ...
      @foreach ($data as $key => $library_resource)
        @php
        ...
        <tr>
          <td class="align-middle text-center text-sm">{{ $library_resource->title }}</td>
          <td class="align-middle text-center text-sm">{{ truncateText($library_resource->desc) }}</td>
          <td class="align-middle text-center text-sm">{{ $library_resource->filename }}</td>
          <td class="align-middle text-center text-sm">{{ $library_resource->path }}</td>
          <td class="align-middle text-center text-sm">{{ truncateText($library_resource->content, 50) }}</td>
          <td class="align-middle text-center text-sm">{{ $library_resource->mime_type }}</td>
          <td class="align-middle text-center text-sm">{{ $library_resource->filesize }}</td>
          <td class="align-middle text-center text-sm">{{ $library_resource->is_active == 1 ? "Active" : "Inactive" }}</td>
          <td class="d-flex justify-content-center align-items-center align-middle text-center text-sm pb-0">
            <a class="btn btn-info" href="{{ asset("https://kll-portal-spaces.sgp1.digitaloceanspaces.com/library_resources/".$library_resource->filename) }}">Show</a>
            <a class="btn btn-primary mr-2" href="{{ route('admin.library-resources.edit', $library_resource->id) }}">Edit</a>
              {!! Form::open(['method' => 'DELETE','route' => ['admin.library-resources.destroy', $library_resource->id],'style'=>'display:inline']) !!}
                @csrf
